...
| **A2 — LLM provider module** | LiteLLM router, `providers.yaml` catalog, per-user encrypted key wiring, `BaseLLMProvider` interface, `TracedLLMProvider` (event emission + cost capture + budget cap + retry/backoff), `MockLLMProvider` for deterministic tests. | Real call against ≥1 provider succeeds and emits `LLMCalled` event with token + cost; budget cap halts execution; mock provider deterministic across runs. | FR-3 (key handling); NFR-8. |
