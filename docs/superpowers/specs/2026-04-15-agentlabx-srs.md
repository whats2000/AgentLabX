...
- **Responsibilities:**
  ...
  - Settings split: per-user (personal API keys, personal MCP servers, budget caps) vs. admin-only (provider catalog, server-wide MCP servers, stage plugin install, LAN-bind toggle, user provisioning).
...
- **Wrapped layers:** `TracedLLMProvider` (event emission), `BaseLLMProvider` (interface). **Test infrastructure:** a standalone OpenAI-compatible mock HTTP server (test-only, not shipped in the production package) so that integration tests exercise the real `LiteLLMProvider` → LiteLLM → HTTP pipeline rather than bypassing it with a fake class.
...
- **SQLite (`agentlabx.db`):**
  ...
  - `sessions` — issued session tokens bound to an `Auther` identity, with expiry, last-seen, and revocation metadata. Tokens never carry credential material.
  - `capabilities` — capability assignments per identity (e.g., `admin`, `memory_curator`).
...
|-------|-------|--------------|--------------|
| **A1 — Foundation infrastructure** | Auth (`Auther` Protocol + `DefaultAuther` / `TokenAuther` / `OAuthAuther`), session layer (cookies, loopback/LAN bind, TLS), admin capability + per-user split, encrypted user config (Fernet + OS keyring), event bus skeleton, plugin registry skeleton, FastAPI server shell with bind config, SQLite schema bootstrap (`users`, `user_configs`, `oauth_tokens`, `app_state`, `sessions`, `capabilities`). **Minimal React shell** — onboarding/login, settings split (per-user vs. admin), API-key entry — sufficient for an admin to log in and register credentials. **No graph rendering, no dashboard beyond a run list, no inner stage views.** | Install fresh, create admin identity, store credential, restart, retrieve credential; second identity isolated under adversarial REST probing; LAN-bind requires TLS. | FR-3, FR-9, FR-15, FR-16; NFR-1, NFR-2, NFR-7, NFR-9; partial FR-5, FR-8. |
| **A2 — LLM provider module** | LiteLLM router, `providers.yaml` catalog, per-user encrypted key wiring, `BaseLLMProvider` interface, `TracedLLMProvider` (event emission + cost capture + budget cap + retry/backoff), test-only mock LLM HTTP server (OpenAI-compatible, exercises real `LiteLLMProvider` → LiteLLM → HTTP pipeline) for deterministic tests. | Real call against ≥1 provider succeeds and emits `LLMCalled` event with token + cost; budget cap halts execution; mock server deterministic across runs. | FR-3 (key handling); NFR-8. |
...
| **A4 — Stage contract framework (all contracts upfront)** | `Stage` Protocol; **input + output Pydantic contracts for every stage in the pipeline** (`literature_review`, `plan_formulation`, `data_exploration`, `data_preparation`, `experimentation`, `interpretation`, `report_writing`, `peer_review`); reproducibility-contract dataclass; plugin discovery for stage implementations; stage I/O validator; backtrack-signal type (target stage + partial-rollback hint). **No stage implementations yet — this is the contract-of-contracts that makes Layer B drift-free.** A stage's internal flow is not specified here; the stage implementation owns it. | Trivial echo-stub registered for every contract via entry points; validator accepts conforming output, rejects schema violations and missing reproducibility fields. | FR-1, FR-2, FR-7. |
| **A5 — RAG component (isolated)** | Chroma local store, three-index design (per-project paper corpus, per-lab reference library, per-project artifact index), chunker, embedder (sentence-transformers default + provider embeddings option), retrieval API, **citation verifier** (rejects ungrounded claims). Pure module — any stage may call it; literature_review will be the first user, but interpretation/report use it too. | Ingest a real PDF; query returns chunks with source attribution; citation verifier accepts a grounded claim and rejects an ungrounded one; per-index isolation enforced. | FR-10. |
| **A6 — Orchestrator + traffic engine + zones** | Zone state machine (Discovery / Implementation / Synthesis as **grouping/labeling only — routing happens at stage level**, per Figure 1); forward routing using stage contracts; backtrack edge tracking (`state["backtrack_attempts"]` keyed `origin->target`, attempt counts); partial rollback executes per the stage's declared backtrack policy; per-edge retry caps; run persistence to SQLite (including per-run **`mode`** field: `auto` or `hitl`); **checkpoint+resume**; **assist-mode backend API** (stage state inspection, user-supplied fix application, resume from checkpoint — the UI panel is Layer C); graph extraction API (`compiled.get_graph()` and `get_graph(xray=1)` for inner subgraphs, consumed later by the frontend); **PI/User decision-call hook** — a backend REST/event surface stages invoke when they need a final decision or clarification (see A8). The orchestrator does **not** itself fire PI at routine points; it only provides the hook that stages choose to call. | 3-stage mock pipeline runs forward; explicit `go_back` from stage B to A succeeds with partial rollback preserving A's prior work; backtrack edge surfaces in graph extraction API with attempt count; checkpoint-resume after kill bit-identical; assist-mode API accepts user-supplied fix payload via REST and resumes; run's `mode` persists and is respected by A8 when the decision-call hook fires. | FR-6, FR-14; completes FR-5. |
...
- **Admin capability** — an `Auther` identity capability authorising server-wide settings (stage plugin install, MCP server registration, provider catalog, LAN-bind toggle, user provisioning). On a fresh solo install the first identity is admin by default. Distinct from `memory_curator`.
