...
| **A2 — LLM provider module** | LiteLLM router, per-user encrypted key wiring (provider derived from model via `litellm.get_llm_provider()`), `BaseLLMProvider` interface, `TracedLLMProvider` (event emission + cost capture + budget cap + retry/backoff), test-only mock LLM HTTP server (OpenAI-compatible, exercises real `LiteLLMProvider` → LiteLLM → HTTP pipeline) for deterministic tests. | Real call against ≥1 provider succeeds and emits `LLMCalled` event with token + cost; budget cap halts execution; mock server deterministic across runs. | FR-3 (key handling); NFR-8. |
